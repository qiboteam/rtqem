...
parser.add_argument("--example", default="hdw_target", help="Target example.", type=str)
...
def main(example):
    ...
    for p in range(len(platforms)):
        plt.fill_between(
            x, means[p] - stds[p], means[p] + stds[p], color=colors[p], alpha=0.3
        )
        plt.plot(
            x, means[p], color=colors[p], alpha=0.7, label=f"Run on {platforms[p]}"
        )
    plt.legend()
    plt.grid(True)
